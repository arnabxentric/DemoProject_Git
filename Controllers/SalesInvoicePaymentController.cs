...
{
    public class SalesInvoicePaymentController : Controller
    {
        //
        // GET: /BillPayment/
        ...
        [HttpGet]
        public ActionResult CreateInvoicePayment(string Msg, string Err, int id)
        {
            if (Msg != null)
            {
                ViewBag.Message = Msg;
            ...
            try
            {
                ...
                var purchase = db.SalesInvoices.Where(d => d.Id == id).FirstOrDefault();


                try
                {
                    string idd = Convert.ToString(id);
                    var lesspayment = db.ReceiptPayments.Where(d => d.transactionNo == idd && d.transactionType == "Sales Invoice").ToList();
                ...
                var purchasedetails = db.SalesInvoiceDetails.Where(d => d.SalesInvoiceId == id);
                ...
                var supplier = db.Customers.Where(c => c.Id == purchase.CustomerId).ToList();
                ViewBag.supplierdet = supplier;




                var paymentdone = db.ReceiptPayments.Where(d => d.transactionNo == id.ToString() && d.transactionType == "Sales Invoice");
        ...
        [HttpPost]
        public ActionResult CreateInvoicePayment(ReceiptPayment receive, FormCollection collection)
        {
            ...
            using (scope)
            {
                ...
                try
                {
                    ...
                    var purchase = db.SalesInvoices.Where(d => d.Id == Invoiceid).FirstOrDefault();
                    var vendorledger = db.Customers.FirstOrDefault(l => l.Id == purchase.CustomerId).LId;
                    ...
                    recpayment.ledgerId = (int)vendorledger;
                    ...
                    db.SaveChanges();
                    scope.Complete();
                    return RedirectToAction("CreateInvoicePayment", new { Msg = "Data Saved Successfully..", id = Invoiceid });
                }
                ...
                catch (DbEntityValidationException e)
                {
                    ...
                    return RedirectToAction("CreateInvoicePayment", new { Err = "Error:Please Try Again!", id = Invoiceid });
