...
namespace XenERP.Controllers
{
    public class SupplierPaymentController : Controller
    {
        ...
        [HttpGet]
        public ActionResult CreatePurchaseReceipt(string Msg, string Err, long? id = 0)
        {
            if (Msg != null)
            {
                ViewBag.Message = Msg;
            ...
            var SuplierList = from cus in db.Suppliers
            ...
            ViewBag.Customer = SuplierList.ToList().OrderBy(d => d.Name);
        ...
        [HttpPost]
        public ActionResult CreatePurchaseReceipt(ReceiptPayment receive, FormCollection collection)
        {
            ...
            using (scope)
            {
                ...
                try
                {
                    ...
                    var cusid = db.Suppliers.Where(d => d.Id == custid).FirstOrDefault();
                    ...
                    db.SaveChanges();
                    scope.Complete();
                    return RedirectToAction("CreatePurchaseReceipt", new { Msg = "   Payment Done Successfully.." });
                }
                ...
                catch (DbEntityValidationException e)
                {
                    ...
                    return RedirectToAction("CreatePurchaseReceipt", new { Err = "Error:Please Try Again!" });
        ...
        public JsonResult getCustomer(string query = "")
        {
            ...
            long Branchid = Convert.ToInt64(Session["BranchId"]);
            var customers = db.Suppliers.Where(p => p.Code.Contains(query) || p.Name.Contains(query) && p.CompanyId == companyid && p.BranchId == Branchid).Select(p => new { Code = p.Code, Id = p.Id, Name = p.Name }).ToList();
        ...
        public JsonResult checkCustomer(string query = "")
        {
            long companyid = Convert.ToInt32(Session["companyid"]);
            long Branchid = Convert.ToInt64(Session["BranchId"]);
            var customers = db.Suppliers.Where(s => (s.Code == query || s.Name == query) && s.CompanyId == companyid && s.BranchId == Branchid).Select(p => new { Name = p.Name, TaxId = p.TaxId, Code = p.Code, CurrencyId = p.CurrencyId, CurrencyPurchaseRate = p.CurrencyRate.PurchaseRate, CurrencySaleRate = p.CurrencyRate.SellRate, CurrencyCode = p.CurrencyRate.Currency.ISO_4217, CurrencyName = p.CurrencyRate.Currency.Currency1, Discount = p.Discount }).FirstOrDefault();
            return Json(customers, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult CreteSupplierPDF(int id, int comids)
        {
            ...
            var customer = db.Suppliers.Where(c => c.Id == id).ToList();
        ...
        public ActionResult PrintPurchasePDF(int id, int comids)
        {


            return new ActionAsPdf("CreteSupplierPDF", new { id = id, comids = comids }) { FileName = "PurchaseReceivePayment.pdf" };
        }
