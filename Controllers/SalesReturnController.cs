...
namespace XenERP.Controllers
{
    public class SalesReturnController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            if (Branchid==0)
                return View(db.SalesReturns.Where(p => p.CompanyId == companyid && p.FinancialYearId == Fyid).ToList());
            else
                return View(db.SalesReturns.Where(p => p.CompanyId == companyid && p.BranchId == Branchid && p.FinancialYearId == Fyid).ToList());
        }
        ...
        public ActionResult Create(long? id = 0)
        {
            ...
            if (id == 0)
            {
                SalesReturnModelView pomv = new SalesReturnModelView();
            ...
            {
                SalesReturn po = db.SalesReturns.Find(id);
                if (po == null)
                {
                    return HttpNotFound();
                }
                SalesReturnModelView pomv = new SalesReturnModelView();
                List<SalesReturnDetailModelView> podmvList = new List<SalesReturnDetailModelView>();
                ...
                var podlist = db.SalesReturnDetails.Where(p => p.SalesReturnId == po.Id).ToList();
                foreach (var pod in podlist)
                {
                    var podmv = new SalesReturnDetailModelView();
                    podmv.SalesReturnId = pod.SalesReturnId;
        ...
        public ActionResult Create(FormCollection poCollection)
        {
            ...
            pomv.OrderNo = poCollection["OrderNo"].ToString().Trim();
            ...
            if (pomv.Type == "1")
            {
                ...
                {
                    SalesReturnDetailModelView podmv = new SalesReturnDetailModelView();
                    podmvList.Add(podmv);
                }
            ...
            if (!(date >= getDateRange.sDate && date <= getDateRange.eDate))
            {
                ...
                ViewBag.ddlWarehouses = warehouses;
                ViewBag.podmvList = podmvList;
                ViewBag.Date = "Sales Return Date out of scope of " + getDateRange.Year + " Financial Year.";
                return View(pomv);
            }
            ...
            var getInvoice = db.SalesInvoices.Where(d => d.CustomerId == pomv.CustomerId && d.NO == pomv.OrderNo).FirstOrDefault();
            if (getInvoice != null)
                pomv.ReferenceNo = getInvoice.Id;
            else
                pomv.ReferenceNo = null;
                ...
                using (scope)
                {
                    ...
                    using (var ctx = new InventoryEntities())
                    {
                        ...
                        if (pomv.Id == 0)
                        {
                            ...
                            pomv.NO = tc.GenerateCode("SR", countpo);
                            ...
                            db.SaveChanges();
                            var po1 = db.SalesReturns.Find(po.Id);
                            pomv.Id = po.Id;
                            if (pomv.Type == "1")
                            {
                                decimal? subtotal = 0;
                                    ...
                                    if (podmv.ItemId != 0 && podmv.Quantity > 0 && podmv.Price > 0 && podmv.TaxId != 0)
                                    {
                                        ...
                                        if (taxComps.Count != 0)
                                        {
                                            ...
                                            foreach (var taxComp in taxComps)
                                            {
                                                ...
                                                var SalesReturnTax = new SalesReturnTax();
                                                SalesReturnTax.SalesReturnId = pomv.Id;
                                                SalesReturnTax.ItemId = podmv.ItemId;
                                                SalesReturnTax.TaxId = (long)taxComp.TaxCompId;
                                                SalesReturnTax.Amount = (decimal)amount1;
                                                SalesReturnTax.CurrencyRate = pomv.Currencyrate;
                                                if (po.Status == "Saved")
                                                {
                                                    db.SalesReturnTaxes.Add(SalesReturnTax);
                                                }
                                            }
                                        ...
                                        else
                                        {
                                            ...
                                            SalesReturnTax.SalesReturnId = pomv.Id;
                                            SalesReturnTax.ItemId = podmv.ItemId;
                                            SalesReturnTax.TaxId = podmv.TaxId;
                                            SalesReturnTax.Amount = (decimal)taxrate * (decimal)compounded / 100;
                                            SalesReturnTax.CurrencyRate = pomv.Currencyrate;
                                            if (po.Status == "Saved")
                                            {
                                                db.SalesReturnTaxes.Add(SalesReturnTax);
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    SalesReturnTax.SalesReturnId = pomv.Id;
                                    // SalesReturnTax.ItemId = podmv.ItemId;
                                    SalesReturnTax.TaxId = (long)pomv.TaxId;
                                    SalesReturnTax.Amount = (decimal)taxrate * (decimal)compounded / 100;
                                    SalesReturnTax.CurrencyRate = pomv.Currencyrate;
                                    if (po.Status == "Saved")
                                    {
                        ...
                        else
                        {
                            ...
                            var po = db.SalesReturns.Find(pomv.Id);
                            ...
                            po.PaymentTermId = pomv.PaymentTermId;
                            po.Memo = pomv.Memo;
                            var podOldRecords = db.SalesReturnDetails.Where(p => p.SalesReturnId == po.Id).ToList();
                            foreach (var podOld in podOldRecords)
                            {
                                db.SalesReturnDetails.Remove(podOld);
                            }
                            var SalesReturnTaxOldRecords = db.SalesReturnTaxes.Where(p => p.SalesReturnId == po.Id).ToList();
                            foreach (var podOld in SalesReturnTaxOldRecords)
                            {
                                db.SalesReturnTaxes.Remove(podOld);
                            }
            ...
            ViewBag.podmvList = podmvList;
            ViewBag.Message = "You have successfully " + pomv.Status + " Sales Return/Credit Note" + pomv.NO;
        ...
        public ActionResult Edit(long id = 0)
        {
            ...
            SalesReturnModelView pomv = new SalesReturnModelView();
            List<SalesReturnDetailModelView> podmvList = new List<SalesReturnDetailModelView>();
            var Customer = db.Customers.Where(s => s.Id == po.CustomerId).Select(s => new { Name = s.Name, Code = s.Code }).FirstOrDefault();
            pomv.NO = po.NO;
            pomv.CustomerCode = Customer.Code;
            pomv.CustomerName = Customer.Name;
            pomv.CustomerId = po.CustomerId;
            pomv.Reference = po.Reference;
        ...
        [HttpPost]
        public ActionResult Edit(SalesReturn SalesReturn)
        ...
        {
            SalesReturn SalesReturn = db.SalesReturns.Find(id);
            var saledetail = db.SalesReturnDetails.Where(d => d.SalesReturnId == SalesReturn.Id);
            foreach (var sid in saledetail)
            {
                db.SalesReturnDetails.Remove(sid);
            }
            var salesTax = db.SalesReturnTaxes.Where(d => d.SalesReturnId == SalesReturn.Id);
            foreach (var sit in salesTax)
            {
                db.SalesReturnTaxes.Remove(sit);
            }
            var stock = db.Stocks.Where(d => d.TranId == SalesReturn.Id);
